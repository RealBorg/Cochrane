package Cochrane::Graph::MagFluc;
...
sub new {
    ...
    $graph->set(dclrs => [qw/red green green green/]);
    $graph->set(line_types => [ 1, 3, 3, 3 ]);
    my $data;
    FILE: for my $file (Cochrane::Store::Acceleration->list()) {
        ...
        if (my $s = Cochrane::Store::Acceleration->get($file)) {
            ...
            SAMPLE: for my $sa (@{$s}) {
                ...
                push @{$data->[1]}, $sa->{m}->{sqe};
                push @{$data->[2]}, $sa->{m_x}->{sqe};
                push @{$data->[3]}, $sa->{m_y}->{sqe};
                push @{$data->[4]}, $sa->{m_z}->{sqe};
            }
        }
    }
